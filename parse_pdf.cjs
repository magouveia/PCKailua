...
Representa os pontos de cultura na forma como trabalha e interage com a equipa 
e clientes, garantindo cuidado com imagem, higiene, comunicação e atenção ao 
detalhe na experiência do cliente. 
Critérios Técnicos 
...
Questionário de Avaliação de Desempenho 
AJUDANTE DE TODAS AS SECÇÕES (Manutenção)
Critério Organizacional 
Representa os pontos de cultura na forma como trabalha e interage com a equipa 
e clientes, garantindo cuidado com imagem, higiene, comunicação e atenção ao 
detalhe na experiência do cliente. 
...
Questionário de Avaliação de Desempenho 
ESTAGIÁRIO / AJUDANTE DE COZINHA 
Critério Organizacional 
Representa os pontos de cultura na forma como trabalha e interage com a equipa 
e clientes, garantindo cuidado com imagem, higiene, comunicação e atenção ao 
detalhe na experiência do cliente. 
...
Critérios Comportamentais – Soft Skills 
...
const blocks = text.split('Questionário de Avaliação de Desempenho').filter(b => b.trim().length > 0);

const result = blocks.map(block => {
  const lines = block.trim().split('\n').map(l => l.trim()).filter(l => l.length > 0);
  
  // Title is everything before "Critério Organizacional"
  let titleLines = [];
  let i = 0;
  while (i < lines.length && !lines[i].startsWith('Critério Organizacional')) {
    titleLines.push(lines[i]);
    i++;
  }
  
  let title = titleLines.join(' ');
  // Fix some titles to match functionalProfiles
  if (title === 'COPEIRO Júnior (até 2 anos)') title = 'Copeiro Júnior (até 2 anos)';
  if (title === 'COPEIRO Sénior (mais de 2 anos)') title = 'Copeiro Sénior (mais de 2 anos)';
  if (title === 'CHEFE DE COPA') title = 'Chefe de Copa';
  if (title === 'TÉCNICO DE LIMPEZA') title = 'Técnico de Limpeza';
  if (title === 'ENCARREGADO DE LIMPEZA') title = 'Encarregado de Limpeza';
  if (title === 'AJUDANTE DE TODAS AS SECÇÕES (Armazém)') title = 'Ajudante de Todas as Secções (Armazém)';
  if (title === 'ASSISTENTE DE ARMAZÉM') title = 'Assistente de Armazém';
  if (title === 'FIEL DE ARMAZÉM') title = 'Fiel de Armazém';
  if (title === 'ENCARREGADO DE ARMAZÉM') title = 'Encarregado de Armazém';
  if (title === 'AJUDANTE DE TODAS AS SECÇÕES (Manutenção)') title = 'Ajudante de Todas as Secções (Manutenção)';
  if (title === 'CARPINTEIRO EM GERAL DE 2ª') title = 'Carpinteiro em Geral de 2ª';
  if (title === 'CARPINTEIRO EM GERAL DE 1ª') title = 'Carpinteiro em Geral de 1ª';
  if (title === 'CHEFE DE MANUTENÇÃO') title = 'Chefe de Manutenção';
  if (title === 'CHEFE DE MANUTENÇÃO , CONSERVAÇÃO E SERVIÇOS TÉCNICOS') title = 'Chefe de Manutenção, Conservação e Serviços Técnicos';
  if (title === 'ESTAGIÁRIO / AJUDANTE DE COZINHA') title = 'Estagiário / Ajudante de Cozinha';
  if (title === 'COZINHEIRO DE 3ª') title = 'Cozinheiro de 3ª';
  if (title === 'COZINHEIRO DE 2ª') title = 'Cozinheiro de 2ª';
  if (title === 'PIZZAIOLO') title = 'Pizzaiolo';
  if (title === 'COZINHEIRO DE 1ª') title = 'Cozinheiro de 1ª';
  if (title === 'SUBCHEFE DE COZINHA') title = 'Subchefe de Cozinha';
  if (title === 'CHEFE DE COZINHA') title = 'Chefe de Cozinha';
  if (title === 'ESTAGIÁRIO / AJUDANTE DE SALA') title = 'Estagiário / Ajudante de Todas as Secções (Sala)'; // Wait, the PDF says ESTAGIÁRIO / AJUDANTE DE SALA, but functionalProfiles says Estagiário / Ajudante de Todas as Secções (Sala)
  if (title === 'ASSISTENTE DE SALA DE 2ª') title = 'Assistente de Sala de 2ª';
  if (title === 'ASSISTENTE DE SALA DE 1ª') title = 'Assistente de Sala de 1ª';
  if (title === 'SUBCHEFE DE SALA') title = 'Subchefe de Sala';
  if (title === 'CHEFE DE SALA') title = 'Chefe de Sala';
  if (title === 'CONTROLADOR-CAIXA') title = 'Controlador-caixa';
  if (title === 'CONTROLADOR') title = 'Controlador';
  if (title === 'CHEFE DE CONTROLO') title = 'Chefe de Controlo';
  if (title === 'ESTAGIÁRIO / AJUDANTE DE BAR') title = 'Estagiário / Ajudante de Todas as Secções (Bar)';
  if (title === 'BARMAN / BARMAID DE 2ª') title = 'Barman / Barmaid de 2ª';
  if (title === 'BARMAN / BARMAID DE 1ª') title = 'Barman / Barmaid de 1ª';
  if (title === 'CHEFE DE BAR') title = 'Chefe de Bar';
  if (title === 'ESTAGIÁRIO / AJUDANTE (Comercial)') title = 'Estagiário / Ajudante de Todas as Secções (Receção)';
  if (title === 'ASSISTENTE DE VENDAS DE 2ª') title = 'Assistente de Vendas de 2ª';
  if (title === 'RECECIONISTA DE 2ª') title = 'Rececionista de 2ª';
  if (title === 'ASSISTENTE DE VENDAS DE 1ª') title = 'Assistente de Vendas de 1ª';
  if (title === 'RECECIONISTA DE 1ª') title = 'Rececionista de 1ª';
  if (title === 'SUBGERENTE') title = 'Subgerente';
  if (title === 'GERENTE') title = 'Gerente';
  if (title === 'SUPERVISOR DE OPERAÇÕES') title = 'Supervisor de Operações';
  if (title === 'ASSISTENTE DE DIREÇÃO') title = 'Assistente de Direção';
  if (title === 'DIRETOR DE RESTAURAÇÃO E BEBIDAS') title = 'Diretor de Restauração e Bebidas';
  if (title === 'ESCRITURÁRIO DE 3ª') title = 'Escriturário de 3ª';
  if (title === 'ESCRITURÁRIO DE 2ª') title = 'Escriturário de 2ª';
  if (title === 'ESCRITURÁRIO DE 1ª') title = 'Escriturário de 1ª';
  if (title === 'CHEFE DE SECÇÃO') title = 'Chefe de Secção';
  if (title === 'DIRETOR DE SERVIÇOS (Administrativos e Financeiros)') title = 'Diretor de Serviços (Administrativos e Financeiros)';
  if (title === 'DIRETOR DE SERVIÇOS TÉCNICOS (Recursos Humanos)') title = 'Diretor de Serviços Técnicos (Recursos Humanos)';

  // Extract sections
  let orgIndex = lines.findIndex(l => l.startsWith('Critério Organizacional'));
  let techIndex = lines.findIndex(l => l.startsWith('Critérios Técnicos'));
  let compIndex = lines.findIndex(l => l.startsWith('Critérios Comportamentais'));
  
  let orgText = lines.slice(orgIndex + 1, techIndex).join(' ');
  let techItems = lines.slice(techIndex + 1, compIndex);
  let compItems = lines.slice(compIndex + 1);
  
  return {
    title,
    organizational: orgText,
    technical: techItems,
    behavioral: compItems
  };
});

const output = `export interface PerformanceCriteria {
  title: string;
...
export const performanceIntro = {
  ...
  criteriaTypes: [
    ...
    { type: "Técnicos", weight: "60%", description: "Avaliam a capacidade para executar corretamente as tarefas da sua função." },
...
export const performanceCriteriaData: PerformanceCriteria[] = ${JSON.stringify(result, null, 2)};
`;

fs.writeFileSync('src/data/performanceCriteria.ts', output);
